...
class _CategoriesScreenState extends State<CategoriesScreen> with SingleTickerProviderStateMixin {
 late AnimationController _animationController;  // late keyword tells dart that this in the end is a variable which will have a value as soon as it's being used the first time, but not yet when the class is created.So, that there simply is a little timing difference between when this property is created technically and when it have initial value and when it will be needed for the first time
 ...
 void _selectCategory(BuildContext context, Category category) {
   final filteredMeals = widget.availableMeals.where( (meal) => meal.categories.contains(category.id) ).toList();  // here the type of filteredMeals is List<Meal>
      ...
      child: GridView(
        ...
        children: [
          ...
          for(final category in availableCategories)
            CategoryGridItem(
              category: category,
              onSelectCategory: _selectCategory,  // or  (){ _selectCategory(context, category); },
            ),
        ],
      ),
